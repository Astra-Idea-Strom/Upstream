...
export const AgentWorkspace: React.FC = () => {
  ...
  return (
    <div className="flex h-full flex-1 flex-col overflow-hidden bg-[#F8F6FE]">
      ...
      <div className="z-20 flex h-11 flex-shrink-0 items-center gap-2 border-b border-slate-200/90 bg-white px-3.5">
        ...
        <span className="truncate text-2xs text-slate-400">
          {hasAnyCard ? `${progress.completedCount} of ${progress.total}` : 'Empty'}
        </span>
      </div>
          ...
          <div className="mx-auto max-w-3xl space-y-5 pb-20">
            ...
            {hasConfirmedIndustry && (
              <StepSlot
                pending={
                  isLoadingNames ? (
                    <SkeletonCard label="Generating names…" />
                  ) : (
                    agentPending('name', 'Choosing a name…')
                  )
                }
            ...
            {hasConfirmedTagline && (
              <StepSlot
                pending={agentPending('visual', 'Building the palette…')}
            ...
            {hasConfirmedPalette && (
              <StepSlot
                pending={agentPending('logo', 'Drawing the mark…')}
 ...
 * Empty canvas.
 *
 * States what the canvas is for, in one line. The five artefacts it will hold
 * are already listed in the top-bar step rail, so repeating them here was pure
 * duplication.
 */
const CanvasEmptyState: React.FC = () => (
  <div className="mx-auto flex min-h-full max-w-sm flex-col items-center justify-center py-12 text-center">
    ...
    <p className="mt-1.5 text-xs leading-relaxed text-slate-500">
      Describe your venture in the panel on the left to begin.
    </p>
  </div>
);
